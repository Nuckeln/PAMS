...
def filterFehlverladungen():
    df = pd.read_excel('/Users/martinwolf/Python/Superdepot Reporting/data/fehlverladungen.xlsx')
    stati = df['Status'].unique()
    bereiche = df['Bereich'].unique()
    typ = df['Typ'].unique()
    version = df['V'].unique()
...
def fehlverladungAnzeigen():
    dfbestand = filterFehlverladungen()
    #dfbestand = pd.read_csv('/Users/martinwolf/Python/Superdepot Reporting/data/fehlverladung.csv', sep=',', encoding='utf-8')
    #dfbestand = pd.read_excel('/Users/martinwolf/Python/Superdepot Reporting/data/fehlverladungen.xlsx', engine='openpyxl')
    df = dfbestand
    #df['ID'] = df.index
...
# Detaildaten anzeigen   
    ...
    with st.expander("Mail", expanded=False):
        mailAnzeigen(mailfile)
    fehlverladungImDetail(selid)

def fehlverladungDashboard():
    st.title('Fehlverladungen')
    dfbestand = filterFehlverladungen()
