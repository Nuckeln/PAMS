...
import os
import datetime
from Data_Class import SynapseReader  # Assuming read_synapse.py is in the same directory
# --- CONFIGURATION & LOGOS ---
...
def load_data():
    ...
    try:    
        ...
        # New mapping based on VIEW_SFG_SAPECC_WhatsNext
        rename_map = {
            'Bestellung_Order': 'EBELN',
            'Bestell_Menge_STO': 'Order_Quantity',
            'Freier_Bestand_DE06_Summe_in_KG': 'Free_Stock_DE06_Sum',
            'Menge_in_Palette': 'Quantity_in_D97_Pallet',
            'Menge_in_CS': 'Quantity_in_CS_Case',
            'Bestell_Einheit': 'Order_Unit',
            'Empfangs_Werk': 'Receiving_Plant',
            'Materialbezeichnung': 'Article_Text',
        ...
        if 'LoadingDateExBayreuth' in df.columns:
        ...
        # Clean numbers (Handle German decimal comma)
        ...
        if 'Shippingconditions_Effective' in df.columns:
            ...
            with st.container(border=True):
                ...
                if df_area.empty:
                    ...
                    grouped = df_area.groupby('EBELN').agg(
                        ...
                        Delivery_Note_Number=('Delivery_Note_Number', 'first'),
                        Is_Picked_in_Warehouse=('Is_Picked_in_Warehouse', 'first'),
                        Is_Goods_Issue_Posted=('Is_Goods_Issue_Posted', 'first')
                    ...
                    for _, row in grouped.head(5).iterrows():
                        ...
                        # Get the new status fields
                        delivery_note = row['Delivery_Note_Number'] if pd.notna(row['Delivery_Note_Number']) else 'N/A'
                        is_picked = 'Yes' if str(row['Is_Picked_in_Warehouse']).strip().upper() == 'X' else 'No'
                        is_gi_posted = 'Yes' if str(row['Is_Goods_Issue_Posted']).strip().upper() == 'X' else 'No'
                        ...
                        st.markdown(
                            ...
                            <div style='font-size:12px; margin-bottom:6px; border-bottom:1px solid #f0f0f0; padding-bottom:4px;'>
                                ...
                                <div style='font-size:11px; color:grey; margin-top:4px;'>
                                    DN: <b>{delivery_note}</b> | Picked: <b>{is_picked}</b> | GI Posted: <b>{is_gi_posted}</b>
                                </div>
                            </div>
                            """, 
                            unsafe_allow_html=True
