...
def new_forecast(data, spalte_summe, sel_Zeitraum):
    ...
    def fit_sarima_model(data, order, seasonal_order):
        ...
        depot_data.index = pd.DatetimeIndex(depot_data.index)
        forecast_dates = pd.date_range(start=depot_data.index.max() + pd.Timedelta(days=1), periods=sel_Zeitraum*2, freq='B')
        forecast_dates = [d for d in forecast_dates if not is_bavarian_holiday(d.date())][:sel_Zeitraum]
        ...
        forecast_df['forecastDate'] = forecast_dates
        return forecast_df.set_index('forecastDate')
...
def main():
    try: 
        exestierende_forecasts = SQL.read_table('PAMS_Forecast_SIRAMAX')
    ...
    if neu:
        ...
        save_Table_append(new_F, 'PAMS_Forecast_SIRAMAX')
