...
class AzureStorage:
    ...
    def upload_file_to_blob_storage(filename, file_object):
        ...
        df = pd.DataFrame({'filename': [filename], 'filenameorg': [filenameorg], 'dateTime': [pd.Timestamp.now()]})
    ...
    def get_blob_file(name: str):
        '''Laden einer Datei aus dem Blob-Speicher. Erwartet den Namen der Datei.
        Args:
            name (str): Name der Datei im Blob-Speicher
    ...
    def st_Azure_multi_uploadBtn(herkunft):
        '''erstellt ein St.FileUploader und einen Upload Button für den Blob Storage erwartet die Herkunft/Anwendung der Datei'''
        uploaded_files = st.file_uploader('Datei hochladen', accept_multiple_files=True)
        filenames = []
        if st.button('Upload der Dokumente'):    
            if uploaded_files is not None:
                for uploaded_file in uploaded_files:
                    filename = upload_file_to_blob_storage(uploaded_file.name, uploaded_file, herkunft)
